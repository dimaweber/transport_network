...
{
    for ( const auto& road: roads ) {
        if ( (road.from_city == from && road.to_city == to) || (road.from_city == to && road.to_city == from) ) {
            return true;
        }
    }
    return false;
...
bool DotParser::parse_dot_content (const std::string& content)
{
    ...
    while ( std::getline(stream, line) ) {
        ...
        if ( line.find("rankdir") != std::string::npos || line.find("node [") != std::string::npos || line.find("edge [") != std::string::npos || line.find("subgraph") != std::string::npos
...
GoodsType DotParser::string_to_goods_type (const std::string& str)
{
    ...
    if ( lower_str == "iron" )
        return GoodsType::Iron;
    if ( lower_str == "silk" )
        return GoodsType::Silk;
...
TruckType DotParser::string_to_truck_type (const std::string& str)
{
    ...
    if ( upper_str == "FORD" )
        return TruckType::Ford;
    if ( upper_str == "MAN" )
        return TruckType::Man;
    if ( upper_str == "VW" )
        return TruckType::Vw;
...
void DotParser::print_network_summary (std::ostream& os) const
{
    ...
    for ( const auto& [name, city]: network_.cities ) {
        ...
        for ( const auto& good: city->warehouse.goods ) {
            os << "    " << DotParser::goods_type_to_string(good.type) << ": " << good.quantity_tons << "t\n";
        }
        os << "  Fleet:\n";
        for ( const auto& truck: city->fleet.trucks ) {
            os << "    " << DotParser::truck_type_to_string(truck.type) << ": " << truck.count << " units\n";
        }
        os << "\n";
    }
    ...
    for ( const auto& road: network_.roads ) {
        os << "  " << road.from_city << " -> " << road.to_city << " (" << road.name << ")\n";
        os << "    Max mass: " << road.max_mass_tons << "t, Max speed: " << road.max_speed_kmh << "km/h\n";
    }
}
...
bool save_network_to_dot (const TransportNetwork& network, const std::string& filename)
{
    ...
    for ( const auto& [name, city]: network.cities ) {
        ...
        file << "Warehouse:\\n";
        for ( const auto& good: city->warehouse.goods ) {
            file << DotParser::goods_type_to_string(good.type) << ": " << good.quantity_tons << "t\\n";
        }
        file << "\\nTrucks:\\n";
        for ( const auto& truck: city->fleet.trucks ) {
            file << DotParser::truck_type_to_string(truck.type) << ": " << truck.count << " units\\n";
        }
        file << "\"];\n";
    }
    ...
    for ( const auto& road: network.roads ) {
        ...
        if ( road.type == RoadType::Alternative )
            color = "green";
        else if ( road.type == RoadType::LongDistance )
            color = "red";
